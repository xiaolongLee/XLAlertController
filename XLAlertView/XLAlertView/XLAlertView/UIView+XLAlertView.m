...
static const NSInteger kTagXLAlertView = 1812;
...
@implementation UIView (XLAlertView)
+ (void)addXLNotifierWithText: (NSString *)text dismissAutomatically:(BOOL)shouldDismiss
{
    ...
    if (notifierView) {
        ...
        if (shouldDismiss) {
            [self performSelector:@selector(dismissXLNotifier) withObject:nil afterDelay:2.0];
        }
    }else {
        ...
        notifierView.tag = kTagXLAlertView;
        ...
        if (shouldDismiss) {
            ...
            [buttonCancel addTarget:self action:@selector(buttonCancelClicked:) forControlEvents:UIControlEventTouchUpInside];
            [buttonCancel setTitle:@"Cancel" forState:UIControlStateNormal];
            buttonCancel.titleLabel.font = NOTIFIER_LABEL_FONT;
            [notifierView addSubview:buttonCancel];
...
+ (UIView * ) checkIfNotifierExistsAlready {
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(dismissXLNotifier) object:nil];
    
    UIView *notifier = nil;
    for (UIView *subview in [APPDELEGATE.window subviews]) {
        if (subview.tag == kTagXLAlertView && [subview isKindOfClass:[UIView class]]) {
            notifier =subview;
        }
    }
...
+ (void) startEntryAnimation : (UIView* ) notifierView withFinalFrame : (CGRect) finalFrame {
    ...
    CATransform3D transform = [self transformWithAxisValue:-0.1 andAngle:45];
    notifierView.layer.zPosition = 400;
    notifierView.layer.transform = transform;
    
    [UIView animateWithDuration:0.2 delay:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        notifierView.frame = finalFrame;
        CATransform3D transform = [self transformWithAxisValue:0.1 andAngle:15];
        notifierView.layer.zPosition = 400;
        notifierView.layer.transform = transform;
    } completion:^(BOOL finished) {
        [UIView animateWithDuration:0.4 delay:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
            ...
            CATransform3D transform = [self transformWithAxisValue:0.0 andAngle:90];
            notifierView.layer.zPosition = 400;
            notifierView.layer.transform = transform;
        } completion:^(BOOL finished) {
...
+ (void) startExitAnimation : (UIView* ) notifierView {
    ...
    [UIView animateWithDuration:0.3 delay:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        CATransform3D transform = [self transformWithAxisValue:0.1 andAngle:30];
        notifierView.layer.zPosition = 400;
        notifierView.layer.transform = transform;
        ...
        [UIView animateWithDuration:0.15 delay:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
            ...
            CATransform3D transform = [self transformWithAxisValue:-1 andAngle:90];
            notifierView.layer.zPosition = 400;
            notifierView.layer.transform = transform;
...
+ (CATransform3D) transformWithAxisValue:(CGFloat) xValue andAngle : (CGFloat) valueOfAngle {
